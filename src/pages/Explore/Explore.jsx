...
const Explore = () => {
  ...
  const getSizeClasses = (size) => {
    switch (size) {
      case 'small': return 'w-full sm:w-64 h-16 sm:h-20';
      case 'medium': return 'w-full sm:w-80 h-16 sm:h-20';
      case 'large': return 'w-full sm:w-96 h-16 sm:h-20';
      default: return 'w-full sm:w-64 h-16 sm:h-20';
    }
  };
  ...
  const MedicineCard = ({ name, rating, price, image }) => (
    <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
      <div className="relative">
        <img src={image} alt={name} className="w-full h-32 sm:h-48 object-cover rounded-t-lg" />
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="flex items-center mb-2">
          {[...Array(5)].map((_, index) => (
            <svg key={index} className={`w-2 h-2 sm:w-3 sm:h-3 ${index < rating ? 'text-yellow-400' : 'text-muted-foreground'}`} fill="currentColor" viewBox="0 0 20 20">
    ...
    <div className="min-h-screen bg-gray-50 py-6 sm:py-8">
      <div className="max-w-[1200px] mx-auto px-3 sm:px-4">
        ...
        <div className="mb-12 sm:mb-16">
          <h1 className="text-2xl sm:text-4xl font-semibold text-gray-800 mb-6 sm:mb-8">Top Categories</h1>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {data.categories.exploreCategories.map((category) => (
              <div
                key={category.id}
                className={`${getSizeClasses(category.size)} bg-gray-200 rounded-full flex items-center justify-between text-gray-800 cursor-pointer px-4 sm:px-6 mx-auto`}
              >
        ...
        {data.categories.exploreCategories.map(category => {
          ...
          return (
            <div key={category.id} className="mb-8 sm:mb-12">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">{category.name}</h2>
